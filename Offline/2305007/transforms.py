...
class ArbitraryLengthFFT(FFTTransformer):
    ...
    def transform(self, x):
        ...
        if N <= 1:
            return x.copy()
        conv_len = next_power_of_two(2 * N - 1)
        
        n = np.arange(N)
        chirp = np.exp(-1j * np.pi * n**2 / N)
        a = np.zeros(conv_len, dtype=np.complex128)
        b = np.zeros(conv_len, dtype=np.complex128)
        a[:N] = x * chirp
        b[:N] = np.conj(chirp) # b[m] = exp(+j*pi*m^2/N) 
        
        # the convolution sum_{n=0}^{N-1} a[n]b[k-n]
        # let m = k-n, then range of m is -(N-1) to (N-1)
        # need to keep the negative indices in b, so we copy b[m] to b[conv_len - m] for m = 1 to N-1 
        # b is symmetric, b[m] = b[-m]
        for m in range(1, N):
            b[conv_len - m] = b[m]    
